...
def object_db_entry(test_db_session):
    ...
    TECHNICAL STRATEGY:
    - It uses 'test_db_session.add()' and 'test_db_session.flush()' instead of 'test_db_session.commit()'.
    - WHY? 'flush()' pushes the data to the database so it generates IDs (Primary Keys),
      but it keeps the database transaction open.
    - This allows the parent 'test_db_session' fixture to roll back the entire transaction after
